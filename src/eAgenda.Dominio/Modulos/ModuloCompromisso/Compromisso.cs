...
public class Compromisso : EntidadeBase<Compromisso>, IEntidadeDoUsuario
{
    ...
    public override List<ErroValidacao> Validar()
    {
        ...
        if (string.IsNullOrWhiteSpace(Assunto) || Assunto.Length < 2 || Assunto.Length > 100)
            erros.Add(new(nameof(Assunto), "O campo \"Título\" deve conter entre 2 e 100 caracteres."));
        ...
        if (Tipo == TipoCompromisso.Presencial && string.IsNullOrWhiteSpace(Local))
            erros.Add(new(nameof(Tipo), "O campo \"Local\" deve ser preenchido para compromissos presenciais."));

        if (Tipo == TipoCompromisso.Remoto && string.IsNullOrWhiteSpace(Link))
            erros.Add(new(nameof(Tipo), "O campo \"Link\" deve ser preenchido para compromissos remotos."));
